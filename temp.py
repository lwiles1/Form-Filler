...
def printFieldsForA1Medical(datafields, can, locations, page):
    ...
    for loc in locations:
        if "Page" in loc.keys() and page == loc["Page"]:
            try:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 5:
        sys.exit("Usage: python3 A1_Medical_Rental_Agreement.py <Google Form CSV> <Coordinates CSV> <Template PDF> <Output PDF>")
    ...
    for i in range(num_pages):
        print(i)
        if i + 1 in pages_to_edit:
            # Initialize canvas to draw form field data
            packet = io.BytesIO()
            can = canvas.Canvas(packet, pagesize=letter)
            
            # Draw all form fields onto the canvas
